...
import com.gowtham.library.ui.VideoBitRateCalculator;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private static long getDuration(MediaMetadataRetriever mediaMetadataRetriever) throws Exception {
        String durationString = mediaMetadataRetriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION);
        if (durationString == null) {
            throw new Exception("Cannot determine duration of video, null meta data");
        }
        try {
            long duration = Long.parseLong(durationString);
            if (duration <= 0) {
                throw new Exception("Cannot determine duration of video, meta data: " + durationString);
            }
            return duration;
        } catch (NumberFormatException e) {
            throw new Exception("Cannot determine duration of video, meta data: " + durationString, e);
        }
    }
    ...
    private void openTrimActivity(String data) {
        if (trimType == 0) {
            try {
                int upperSizeLimit = 100 * MB;
                MediaMetadataRetriever retriever = new MediaMetadataRetriever();
                //String realPath = FileUtils.getRealPath(this, Uri.parse(data));
                String realPath = "/storage/emulated/0/Android/data/com.gowtham.videotrimmer/files/TrimmedVideo/trimmed_video_2022_1_17_20_49_27.mp4";
                retriever.setDataSource(realPath);
                long inSize = new File(realPath).length();
                long duration       = getDuration(retriever);
                int inputBitRate   = VideoBitRateCalculator.bitRate(inSize, duration);
                VideoBitRateCalculator.Quality targetQuality  = new VideoBitRateCalculator(upperSizeLimit).getTargetQuality(duration, inputBitRate);
                boolean transcodeRequired = inputBitRate >= targetQuality.getTargetTotalBitRate() * 1.2 || inSize > upperSizeLimit;

                Log.e("transcodeRequired -->"," " + transcodeRequired);

                String videoBitrate = prettyCount(targetQuality.getTargetVideoBitRate());
                String audioBitrate = String.valueOf(targetQuality.getTargetAudioBitRate());
                int width = Integer.parseInt(retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_WIDTH));
                int height = Integer.parseInt(retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_HEIGHT));
                int videoResolution = targetQuality.getOutputResolution();
                ...
                compressOption.audioBitrate = audioBitrate;
